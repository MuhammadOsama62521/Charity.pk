...
import { Form,
 ...
 class sylani extends Component {
    state = {
        products: [],
        approvedProducts:[],
        //approval:false,
      ...
      handleApprove=(key)=>{
        ...
        firebase.database().ref().child('Sylani').child('product').child(key).update({approval:true,price:this.state.price})
        swal("Approved")
      }
      ...
      handleUnapprove=(key)=>{
        firebase.database().ref().child('Sylani').child('product').child(key).update({approval:false,price:'0'})
        swal("Unapproved")
      }
